...
int izbor(char *);
pozicija dodajDirektorij(pozicija, char *);
int ispisPoddirektorija(pozicija);
pozicija podDirektorij(pozicija, char *, poz);
int staviNaStog(pozicija, poz);
...
int main()
{
	...
	while (strcmp(buff, "exit"))
	{
		...
		case 1://md
			...
			last = dodajDirektorij(last, buff);
			break;
		case 2://cd dir
			memset(buff, 0, 30);
			sscanf(komandna_linija + pos, "%s", buff);
...
pozicija podDirektorij(pozicija lastchild, char *buff, poz stog)
{
	...
	if (staviNaStog(trenutni, stog) != 0)
		printf("Greska!!!\n");

	return trenutni;
}
int staviNaStog(pozicija trenutni, poz stog)
{
	poz NewNode = NULL;
...
int ispisPoddirektorija(pozicija last)
{
	pozicija trenutni = NULL;
...
pozicija dodajDirektorij(pozicija last, char *dirname)
{
